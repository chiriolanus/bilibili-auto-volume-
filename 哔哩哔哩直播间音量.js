...
(function () {
	...
	function buildPanel() {
		...
		panel.innerHTML = `
			<h2>直播间音量</h2>
			<div class="subtitle">为每个直播间单独保存音量，未设置的直播间会自动使用默认音量。</div>

			<div class="section">
				<div class="section-title">默认音量</div>
				<div class="section-desc">当当前直播间没有单独设置时，脚本会自动使用这里的默认值。</div>
				<div class="volume-row">
					<input id="default-volume-range" type="range" min="0" max="100" step="1" value="${state.defaultVolume}">
					<input id="default-volume-number" type="number" min="0" max="100" step="1" value="${state.defaultVolume}">
				</div>
			</div>
			...
			<div class="section">
				...
				<div class="section-desc">可以先调整预览值，再保存到当前直播间；删除后会回退到默认音量。</div>
				<div class="volume-row">
					<input id="room-volume-range" type="range" min="0" max="100" step="1" value="${getTargetVolume(getCurrentRoomId())}">
					<input id="room-volume-number" type="number" min="0" max="100" step="1" value="${getTargetVolume(getCurrentRoomId())}">
				</div>
				<div class="actions">
